...
import { z } from "zod";
import { crawlQueue, isCrawlQueueConfigured } from "../../../../lib/seo-audit/crawl-queue";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = StartCrawlRequest.parse(body);
    const { startUrl, sameHostOnly, maxDepth, timeout } = parsed;
    let { limit } = parsed;
