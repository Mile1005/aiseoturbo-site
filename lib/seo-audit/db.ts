...
export async function getPrisma(): Promise<PrismaClientType> {
  if (!prismaInstance) {
    ...
    const PrismaClient = (prismaModule as Record<string, unknown>).PrismaClient || ((prismaModule as Record<string, unknown>).default as Record<string, unknown>)?.PrismaClient;
    const globalForPrisma = global as unknown as { prisma?: PrismaClientType };
    prismaInstance = globalForPrisma.prisma ?? new (PrismaClient as { new(): PrismaClientType })();
...
export const dbHelpers = {
  async createRun(data: {
    ...
    await prisma.run.create({
      data: {
        ...
        status: (data.status as unknown) || "queued",
      },
    });
  ...
  async updateRunStatus(runId: string, status: string) {
    const prisma = await getPrisma();
    return prisma.run.update({ where: { id: runId }, data: { status: status as unknown } });
  },
  ...
  async getRunsByEmail(email: string, limit: number = 10, offset: number = 0) {
    ...
    const runIds = runs.map((r: unknown) => (r as Record<string, unknown>).id);
    const audits = await prisma.audit.findMany({
      where: { runId: { in: runIds } },
      select: { runId: true },
    });
    const runIdHasAudit = new Set(audits.map((a: unknown) => (a as Record<string, unknown>).runId));

    return {
      runs: runs.map((run: unknown) => ({
        ...(run as Record<string, unknown>),
        hasResults: runIdHasAudit.has((run as Record<string, unknown>).id as string),
      })),
      pagination: {
        total,
        limit,
  ...
  async saveAudit(data: { id: string; runId: string; json: unknown }) {
    const prisma = await getPrisma();
    return prisma.audit.create({ data: { id: data.id, runId: data.runId, json: data.json as unknown } });
  },
