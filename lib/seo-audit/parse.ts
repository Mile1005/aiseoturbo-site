        ...
        items.forEach((item) => {
          if (item && typeof item === "object") {
            ...
            if (item["@graph"] && Array.isArray(item["@graph"])) {
                          item["@graph"].forEach((graphItem: unknown) => {
              const graphItemData = graphItem as Record<string, unknown>;
              if (graphItemData && graphItemData["@type"]) {
                const types = Array.isArray(graphItemData["@type"])
                  ? graphItemData["@type"]
                  : [graphItemData["@type"]];
                jsonLdTypes.push(...types);
              }
