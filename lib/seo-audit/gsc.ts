...
const gscCache = new Map<string, { result: unknown; timestamp: number }>();
...
export async function fetchGscInsightsForUrl(url: string, state?: string): Promise<unknown> {
  // Check cache first
  ...
  try {
    ...
    const topQueries = rows.map((row: unknown) => {
      const rowData = row as Record<string, unknown>;
      totalClicks += (rowData.clicks as number) || 0;
      totalImpressions += (rowData.impressions as number) || 0;
      return {
        query: ((rowData.keys as unknown[])?.[0] as string) || '',
        clicks: (rowData.clicks as number) || 0,
        impressions: (rowData.impressions as number) || 0,
        ctr: (rowData.ctr as number) || 0,
