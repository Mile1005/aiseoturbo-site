...
import { fetchHtml } from "../../lib/seo-audit/scrape";
import { parseHtml } from "../../lib/seo-audit/parse";
import { calculateAudit } from "../../lib/seo-audit/heuristics";
import { fetchPageSpeed } from "../../lib/seo-audit/psi";
import { fetchGscInsightsForUrl } from "../../lib/seo-audit/gsc";
  ...
  try {
    ...
    if (!performanceData) {
      performanceData = {
        ...
        inp: null,
        notes: [
          psiError ? `PSI error: ${(psiError as Error).message}` : 'Performance data not available',
        ],
      };
    }
    ...
    if (!gscData) {
      gscData = {
        ...
        impressions: null,
        clicks: null,
        message: gscError ? `GSC error: ${(gscError as Error).message}` : 'GSC data not available',
      };
    }
